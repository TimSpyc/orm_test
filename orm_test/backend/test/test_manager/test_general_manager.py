...
class TestFilter(TestCase):
    ...
    def test_filter(self):
        result = self.manager.filter(name = 'TestProject1')
        expected_result = [
            TestProjectManager(test_project_group_id= self.test_project_group.id)
        ]
        self.assertEqual(result, expected_result)
        
    def test_filter_with_multiple_results(self):
        result = self.manager.filter(project_number='123456')

        expected_result = [
            TestProjectManager(test_project_group_id= self.test_project_group.id),
            TestProjectManager(test_project_group_id= self.test_project_group2.id)
        ]
        self.assertEqual(result, expected_result)
        
    # def test_filter_with_conditions_in_list(self):
    #     result = self.manager.filter(name = ['TestProject1', 'TestProject2'])
    ...
    def test_filter_with_search_date(self):
        search_date = datetime(2023, 5, 13)
    ...
    def test_filter_with_dict_of_conditions(self):  
        dict = {'name' : 'TestProject1','project_number' : '123456'}

        result = self.manager.filter(**dict)
        expected_result = [
            TestProjectManager(test_project_group_id=self.test_project_group.id)
        ]
        self.assertEqual(result, expected_result)
