...
class TestGetValueForReferencedModelById(TestCase):
    ...
    def test_invalid_model(self):
        with self.assertRaises(NameError):
            value = GeneralManager._GeneralManager__getValueForReverencedModelById(non_existing_model, 'project_group', 1)
    ...
    def test_invalid_id(self):
        with self.assertRaises(NotValidIdError):
            value = GeneralManager._GeneralManager__getValueForReverencedModelById(TestProject, 'project_group', 3)
...
class TestGetValueForManyToManyByIdList(TestCase):
    ...
    def test_invalid_model(self):
        with self.assertRaises(NameError):
            value = GeneralManager._GeneralManager__getValueForManyToManyByIdList(non_existing_model, 'project_user_role', [1,2])

    def test_invalid_id_in_list(self):  
        with self.assertRaises(NotValidIdError):
            value = GeneralManager._GeneralManager__getValueForManyToManyByIdList(TestProjectUser, "project_user_role", [1, 2, 3])

    def test_with_no_given_id_list(self):
        result = GeneralManager._GeneralManager__getValueForManyToManyByIdList(TestProject, 'project_group', [])
...
class TestGetValueAndColumnIfExists(TestCase):
    ...
    def setUpTestData(cls):
        ...
        project_user1.project_user_role.add(project_user_role1, project_user_role2)

        project_user2 = TestProjectUser.objects.create(project_user_group = project_user_group1)
        project_user2.project_user_role.add(project_user_role1)
        
    def test_get_value_and_column_if_exists(self):
        model = TestProject
        column_list = ['name', 'project_number', 'project_group_id']
        ...
        self.assertEqual(result, (True, 'project_group_id', 1))
    ...
    def test_get_value_and_column_if_exists_with_many_to_many_relation(self):
        column_list = ['project_user_group', 'project_user_role']
        value = [1, 2]
        is_in_model, db_column, value = GeneralManager._GeneralManager__getValueAndColumnIfExists('project_user_role_id_list', column_list, TestProjectUser, value)
        self.assertEqual((is_in_model, db_column, list(value)), (True, 'project_user_role', list(TestProjectUserRole.objects.filter(id__in=value))))
...
class TestGetColumnList(TestCase):
    ...
    def test_get_column_list(self):
        ...
        column_list = ['id', 'name', 'project_number', 'project_group', 'date']
...
class TestAll(TestCase):
    def test_all_function(self):
        project_group = TestProjectGroup.objects.create()
        data1 = TestProject.objects.create(name='TestProject1',project_number='123',project_group = project_group)
        data2 = TestProject.objects.create(name='TestProject2',project_number='123456',project_group = project_group)
        data3 = TestProject.objects.create(name='TestProject3',project_number='123456789',project_group = project_group)
